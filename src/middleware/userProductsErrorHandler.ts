...
export class UserProductsErrorHandler {
   ...
   */
  private static handleServiceError(error: UserProductServiceError, operation: string, productId?: string): ApiError {
    switch (error.code) {
      ...
      default:
        return new ApiError(error.statusCode as HttpStatus, error.code as ErrorCode, error.message, error.details);
    }
   ...
   */
  private static handleDatabaseError(error: DatabaseError, operation: string, productId?: string): ApiError {
      ...
      switch (error.originalError.code) {
        ...
        case "PGRST116": // No rows returned from PostgREST
          if (operation === "PUT" || operation === "DELETE") {
            return new ApiError(HttpStatus.NOT_FOUND, ErrorCode.NOT_FOUND, "Product not found or access denied.", {
              productId,
            });
          }
          break;
   ...
   */
  static createValidationErrorResponse(errors: ValidationError[], requestId: string, operation: string): Response {
   ...
   */
  static createRateLimitErrorResponse(requestId: string, retryAfter: string, userType: "demo" | "verified"): Response {
    const message =
      userType === "demo"
        ? "Rate limit exceeded for demo users. Please wait before making more requests."
        : "Rate limit exceeded. Please wait before making more requests.";

    const apiError = new ApiError(HttpStatus.TOO_MANY_REQUESTS, ErrorCode.RATE_LIMIT_EXCEEDED, message, {
  ...
  static createAuthenticationErrorResponse(
    ...
    const apiError = new ApiError(HttpStatus.UNAUTHORIZED, ErrorCode.UNAUTHORIZED, message, details);
  ...
  private static isRetryableError(error: unknown): boolean {
    if (error instanceof ApiError) {
      return [HttpStatus.SERVICE_UNAVAILABLE, HttpStatus.GATEWAY_TIMEOUT, HttpStatus.TOO_MANY_REQUESTS].includes(
        error.statusCode
      );
    }
