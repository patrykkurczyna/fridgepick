...
export class UserProductRepository implements IUserProductRepository {
  ...
  async findByUserId(userId: string, filters: UserProductFilters = {}): Promise<UserProductWithCategory[]> {
    try {
      ...
      const products = (data || []).map((item) => ({
        ...
        category_name: (item.product_categories as unknown).name,
        category_description: (item.product_categories as unknown).description,
      ...
      throw new DatabaseError("Unexpected error occurred while fetching user products", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async countByUserId(
    userId: string,
    filters: Omit<UserProductFilters, "limit" | "offset" | "sort"> = {}
  ): Promise<number> {
    try {
      ...
      let query = this.supabase.from("user_products").select("*", { count: "exact", head: true }).eq("user_id", userId);
      ...
      if (error) {
        ...
        throw new DatabaseError("Failed to count user products", error, error.code || "USER_PRODUCTS_COUNT_ERROR");
      }
      ...
      throw new DatabaseError("Unexpected error occurred while counting user products", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async create(userId: string, productData: CreateUserProductData): Promise<UserProductWithCategory> {
    try {
      ...
      if (error) {
        ...
        throw new DatabaseError("Failed to create user product", error, error.code || "USER_PRODUCT_CREATE_ERROR");
      }

      if (!data) {
        throw new DatabaseError("No data returned after creating user product", null, "CREATE_NO_DATA");
      }
      ...
      const product: UserProductWithCategory = {
        ...
        category_name: (data.product_categories as unknown).name,
        category_description: (data.product_categories as unknown).description,
      };
      ...
      throw new DatabaseError("Unexpected error occurred while creating user product", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async findById(id: string, userId: string): Promise<UserProductWithCategory | null> {
    try {
      ...
      throw new DatabaseError("Unexpected error occurred while finding user product by ID", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async update(id: string, userId: string, productData: UpdateUserProductData): Promise<UserProductWithCategory> {
    try {
      ...
      if (error) {
        if (error.code === "PGRST116") {
          throw new DatabaseError("Product not found or access denied", error, "PRODUCT_NOT_FOUND_OR_FORBIDDEN");
        }
        ...
        throw new DatabaseError("Failed to update user product", error, error.code || "USER_PRODUCT_UPDATE_ERROR");
      }

      if (!data) {
        throw new DatabaseError("No data returned after updating user product", null, "UPDATE_NO_DATA");
      }
      ...
      throw new DatabaseError("Unexpected error occurred while updating user product", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async delete(id: string, userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        ...
        throw new DatabaseError("Failed to delete user product", error, error.code || "USER_PRODUCT_DELETE_ERROR");
      }
      ...
      throw new DatabaseError("Unexpected error occurred while deleting user product", error, "UNEXPECTED_ERROR");
    }
  }
  ...
  async categoryExists(categoryId: number): Promise<boolean> {
    try {
      ...
      const { data, error } = await this.supabase.from("product_categories").select("id").eq("id", categoryId).single();
      ...
      if (error) {
        ...
        throw new DatabaseError("Failed to check category existence", error, error.code || "CATEGORY_EXISTS_ERROR");
      }
      ...
      throw new DatabaseError("Unexpected error occurred while checking category existence", error, "UNEXPECTED_ERROR");
    }
  }
}
