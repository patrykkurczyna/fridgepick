 ...
 */
const validateRegisterForm = (email: string, password: string, confirmPassword: string): string | null => {
  if (!email) {
...
export const RegisterForm: React.FC = () => {
  ...
  const handleInputChange =
    (field: "email" | "password" | "confirmPassword") => (e: React.ChangeEvent<HTMLInputElement>) => {
      setFormState((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const validationError = validateRegisterForm(formState.email, formState.password, formState.confirmPassword);
    ...
    return (
      <div className="space-y-6">
        <div className="bg-green-50 border border-green-200 rounded-md p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-green-900 mb-2">Rejestracja przebiegła pomyślnie!</h3>
          <p className="text-sm text-green-800 mb-4">
            Sprawdź swoją skrzynkę email <strong>{formState.email}</strong> i kliknij link aktywacyjny, aby zweryfikować
            swoje konto.
          </p>
          <p className="text-xs text-green-700">Jeśli nie widzisz emaila, sprawdź folder spam.</p>
        </div>
      ...
      <div>
        ...
        <div className="mt-2 text-xs text-gray-600 space-y-1">
          ...
          <ul className="list-disc list-inside space-y-0.5 ml-2">
            <li className={formState.password.length >= 8 ? "text-green-600" : ""}>Minimum 8 znaków</li>
            <li className={/[a-z]/.test(formState.password) ? "text-green-600" : ""}>Małą literę</li>
            <li className={/[A-Z]/.test(formState.password) ? "text-green-600" : ""}>Wielką literę</li>
            <li className={/\d/.test(formState.password) ? "text-green-600" : ""}>Cyfrę</li>
          </ul>
        </div>
      </div>
