...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="product-form space-y-6">
      {/* Name Field */}
      <FormField label="Nazwa produktu" error={errors.name} required>
        <input
          type="text"
      ...
      {/* Category Field */}
      <FormField label="Kategoria" error={errors.categoryId} required>
        <CategoryDropdown
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label="Ilość" error={errors.quantity} required>
          <input
            type="number"
            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        <FormField label="Jednostka" error={errors.unit} required>
          <UnitSelector
            value={formData.unit}
            onChange={(unit) => {
      ...
      {/* Expiry Date Field */}
      <FormField label="Data ważności" error={errors.expiresAt}>
        <DatePicker
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-blue-800">Wskazówki dotyczące formularza</h3>
