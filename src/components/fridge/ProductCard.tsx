 ...
 */
export const ProductCard: React.FC<ProductCardProps> = ({ product, onEdit, onDelete, variant = "default" }) => {
  ...
  const getExpiryInfo = () => {
    ...
    switch (expiryStatus) {
      case "expired":
        return {
          text: `Wygasł ${Math.abs(daysUntilExpiry ?? 0)} dni temu`,
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-gray-600 truncate ${isCompact ? "text-xs" : "text-sm"}`}>{product.categoryName}</p>
        </div>
      ...
      {/* Data ważności */}
      <div className={`${expiryInfo.className} ${isCompact ? "text-xs" : "text-sm"}`}>{expiryInfo.text}</div>
