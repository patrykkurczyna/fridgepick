...
import type { ProductFormData, ValidationErrors, LoadingState, ValidationResult } from "@/types/product-form";
...
export const useProductForm = (mode: "create" | "edit", productId?: string) => {
   ...
   */
  const validateField = useCallback((field: keyof ProductFormData, value: unknown): string | undefined => {
    switch (field) {
      case "name": {
      ...
      case "unit":
        ...
        if (!UNIT_TYPES.includes(value as unknown)) {
  ...
  const submitForm = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const url = mode === "create" ? "/api/user-products" : `/api/user-products/${productId}`;
  ...
  return {
    ...
    // Actions
    handleFieldChange,
    handleFieldBlur: handleFieldBlur as (field: keyof ProductFormData, value?: unknown) => void,
    submitForm,
    deleteProduct,
    resetForm,
    validateForm,
