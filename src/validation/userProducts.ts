...
export const CreateUserProductSchema = z
  .object({
    name: z.string().min(1, "Product name is required").max(255, "Product name must be at most 255 characters").trim(),
    ...
    quantity: z
      ...
      .refine((val) => val < 1000000, "Quantity cannot exceed 999,999"),

    unit: z.enum(["g", "l", "szt"] as const, {
 ...
 */
export const UserProductsQuerySchema = z
  .object({
    search: z
      .string()
      .optional()
      .transform((val) => val?.trim() || undefined)
      .refine((val) => val === undefined || val.length >= 2, "Search query must be at least 2 characters")
      .refine((val) => val === undefined || val.length <= 100, "Search query must be at most 100 characters"),

    category: z
      .string()
      .optional()
      .transform((val) => (val ? parseInt(val, 10) : undefined))
      .refine((val) => val === undefined || (!isNaN(val) && val > 0), "Category must be a positive integer"),
    ...
    expired: z
      ...
      .refine((val) => val === undefined || typeof val === "boolean", "Expired must be true or false"),
    ...
    offset: z
      ...
      .optional()
      .transform((val) => (val ? parseInt(val, 10) : undefined))
      .refine((val) => val === undefined || (!isNaN(val) && val >= 0), "Offset must be a non-negative integer"),
  })
  .strict();
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      const errors: ValidationError[] = error.errors.map((err) => ({
        field: err.path.join("."),
        message: err.message,
        value: err.path.reduce((obj, path) => (obj as Record<string, unknown>)?.[path], data),
      }));
 ...
 */
export function validateUserProductsQuery(
  queryParams: Record<string, string | string[] | undefined>
): ValidationResult<z.infer<typeof UserProductsQuerySchema>> {
  try {
    ...
    if (error instanceof z.ZodError) {
      const errors: ValidationError[] = error.errors.map((err) => ({
        field: err.path.join("."),
        message: err.message,
        value: err.path.reduce((obj, path) => (obj as Record<string, unknown>)?.[path], queryParams),
      }));
...
): ValidationResult<z.infer<typeof ProductIdSchema>> {
  try {
    ...
    if (error instanceof z.ZodError) {
      const errors: ValidationError[] = error.errors.map((err) => ({
        field: err.path.join("."),
        message: err.message,
        value: err.path.reduce((obj, path) => (obj as Record<string, unknown>)?.[path], params),
      }));
 ...
 */
export function validateCreateUserProductWithBusinessRules(
  data: unknown
): ValidationResult<z.infer<typeof CreateUserProductSchema>> {
 ...
 */
export function validateUpdateUserProductWithBusinessRules(
  data: unknown
): ValidationResult<z.infer<typeof UpdateUserProductSchema>> {
